...
__global__ void AbsDifferenceParallelTaus(cufftReal *d_diff, float *d_buffer, int* d_tau_vector, int tau_count, int frame_count, int width, int height) {
	// Parallel tau version of AbsDifference
...
__global__ void processFFTParallelTaus(cufftComplex *d_data, float *d_fft, int width, int height) {
	// Takes output of cuFFT R2C operation, normalises it (i.e. divides by px count), takes the magnitude and adds it to the accum_array

	int j = threadIdx.x + blockIdx.x * 32;
	int i = threadIdx.y + blockIdx.y * 32;
	// tau index = blockIdx.z

	int size = width * height;
	int frame_size = width * (height / 2 + 1) * sizeof(cufftComplex);
	...
	if (j <= width-1 && i <= height-1) {
		...
		if (j >= sym_w) {
			// real ->  d_data[i*sym_w+(width-j)].x
			// img  -> -d_data[i*sym_w+(width-j)].y
			mag = cuCabsf(d_data[frame_size*blockIdx.z + i*sym_w+(width-j)]) / (float)size;

		} else {
			// real -> d_data[i*sym_w+j].x
			// img  -> d_data[i*sym_w+j].y
			mag = cuCabsf(d_data[frame_size*blockIdx.z + i*sym_w+j]) / (float)size;
		}

		// add to fft_accum
		d_fft[blockIdx.z * size + pos_offset] += mag*mag;
	}
	return;
}
...
void analyseChunk(float *d_chunk_ptr, int frame_count, float *d_out, int *tau_vector, int tau_count, int width, int height, float *debug_buff=NULL) {
	...
	int grid_x = (int) ceil(width/32.0);
	int grid_y = (int) ceil(height/32.0);

	if (grid_x * grid_y > 65536) {
		std::cout << "Image too big, not enough thread blocks" << std::endl;
	}

	int remaining_blocks = 65536 / (grid_x * grid_y);

	bool parallel_tau = false;
	int grid_z = 1;
	...
	int *d_tau_vector; // move tau vector to device
...
void HARDCODEanalyseFFTHost(float *d_in, float *d_out, int norm_factor, int *tau_vector, int tau_count, int width, int height, float *debug_buff=NULL) {
	...
	float q_squared[20] = 	{ 1.        ,   3.6472384 ,   7.94985584,  13.90785234,  21.52122788, 30.78998247,  41.71411612,
								...
								188.70083102, 214.52337611, 242.00130024, 271.13460343, 301.92328566, 400};
	// Generate masks
...
//    }
    ...
    float q_vector[20] = { 1.        ,  1.90977444,  2.81954887,  3.72932331,  4.63909774,
							 ...
							 14.64661654, 15.55639098, 16.46616541, 17.37593985, 200};
    ...
    float q_sq_vector[20] = {  1.        ,   3.6472384 ,   7.94985584,  13.90785234,  21.52122788,
								 ...
								 214.52337611, 242.00130024, 271.13460343, 301.92328566, 400000};
...
int main(int argc, char **argv)
{
    ...
    float q_vector[20] = { 1.        ,  1.90977444,  2.81954887,  3.72932331,  4.63909774,
							  5.54887218,  6.45864662,  7.36842105,  8.27819549,  9.18796992,
							 10.09774436, 11.0075188 , 11.91729323, 12.82706767, 13.73684211,
							 14.64661654, 15.55639098, 16.46616541, 17.37593985, 400};
